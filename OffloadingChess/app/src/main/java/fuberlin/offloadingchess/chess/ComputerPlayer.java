...
public class ComputerPlayer implements Player {
    ...
    public String getCommand(Position pos, boolean drawOffer, List<Position> history) {
        ...
        sc.setGui(gui);
        sc.setOffloadingEngine(offloadingEngine);
        ...
        if ((moves.size == 1) && (canClaimDraw(pos, posHashList, posHashListSize, moves.m[0]) == "")) {
            ...
            Algorithms.setSearch(sc);

            sc.timeLimit(minTimeMillis, maxTimeMillis);

            String bestMSerialized = null;
            try {
                bestMSerialized = offloadingEngine.execute(gui.isForceOffloading(),
                        Algorithms.AlgName.iterativeDeepening,
                        gui.getOffloadingCriteria(),
                        Serializer.serialize(moves),
            ...
            bestM = ((Move) Serializer.deserialize(bestMSerialized));
