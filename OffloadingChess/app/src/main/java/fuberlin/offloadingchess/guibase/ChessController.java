...
package fuberlin.offloadingchess.guibase;
...
import fuberlin.offloadingchess.chess.ComputerPlayer;
import fuberlin.offloadingchess.chess.Game;
import fuberlin.offloadingchess.chess.Game.GameState;
import fuberlin.offloadingchess.chess.HumanPlayer;
import fuberlin.offloadingchess.chess.Move;
import fuberlin.offloadingchess.chess.MoveGen;
import fuberlin.offloadingchess.chess.Piece;
import fuberlin.offloadingchess.chess.Player;
...
import fuberlin.offloadingchess.chess.UndoInfo;
import fuberlin.offloadingchess.offloading.Serializer;
...
public class ChessController {
    ...
    public ChessController(GUIInterface context) {
        this.gui = context;
        this.context = (Context) context;
        listener = new SearchListener();
        thinkingPV = "";
        threadStack = 0;
        this.offloadingEngine = new Engine(this.context);

    }

    public Engine getOffloadingEngine() {
        return offloadingEngine;
    }

    public void setOffloadingServer(String offloadingServer) {
        offloadingEngine.setServer(offloadingServer);
    }
    ...
    public final void newGame(boolean humanIsWhite, int ttLogSize, boolean verbose) {
        ...
        computerPlayer.setOffloadingEngine(offloadingEngine);
        computerPlayer.setGUI(gui);
        ...
        gameId = sdf.format(cal.getTime());

        offloadingEngine.setGameId(gameId);
        ...
        if (game.pos.whiteMove != humanIsWhite) {
            if (computerThread == null) {
                Runnable run = new Runnable() {
                    public void run() {
                        computerPlayer.timeLimit(60000, 60000, false);
                        computerPlayer.setMaxDepth(gui.getMaxDepth());


                        //final String cmd = offloadingEngine.execute(gui.isForceOffloading(),
                                //Algorithms.AlgName.startComputerThinking,
                                //gui.getOffloadingCriteria(),
                                //strOffloadingPackage);
